...
export interface EvalCtx {
  ...
  meta: { time_night: number; update_diligent: number };
}
...
function countDays(predicate: (l: DailyLog) => boolean, logs: DailyLog[]): number {
  let n = 0;
  for (const l of logs) if (predicate(l)) n += 1;
  return n;
}
...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  // ── Walking ──────────────────────────────────────────────
  {
    ...
    progress: (c) => ({
      current: Math.min(
        countDays((l) => completedWalks(l) >= 2, c.logs),
        25,
      ),
      target: 25,
    }),
  },
  {
    ...
    progress: (c) => ({
      current: Math.min(
        countDays((l) => completedWalks(l) >= 3, c.logs),
        10,
      ),
      target: 10,
    }),
  ...
  },
  {
    ...
    progress: (c) => ({
      current: Math.min(
        streakLen((l) => totalWalkMinutes(l.walks) >= DAILY_WALK_TARGET_MIN, c.logs),
        30,
      ),
      target: 30,
    }),
  ...
  // ── Health Management ──────────────────────────────────────────────
  {
    ...
    progress: (c) => ({
      current: Math.min(
        streakLen((l) => isNoIssues(l.symptoms), c.logs),
        14,
      ),
      target: 14,
    }),
  ...
  // ── Nutrition ──────────────────────────────────────────────
  {
    ...
    progress: (c) => ({
      current: Math.min(
        streakLen((l) => l.dins_percent === 100, c.logs),
        14,
      ),
      target: 14,
    }),
  ...
  },
  {
    ...
    progress: (c) => ({
      current: Math.min(
        countDays((l) => l.dins_percent === 100, c.logs),
        100,
      ),
      target: 100,
    }),
  ...
  },
  {
    ...
    progress: (c) => ({
      current: Math.min(
        streakLen((l) => !!l.holiday_mode, c.logs),
        14,
      ),
      target: 14,
    }),
  ...
  // ── Surprise Milestones ──────────────────────────────────────────────
  {
    ...
    category: "Surprise Milestones",
    icon: "HeartPulse",
    color: "oklch(0.65 0.18 145)",
    evaluate: (c) => streakLen((l) => l.health_score === 3, c.logs) >= 7,
    progress: (c) => ({
      current: Math.min(
        streakLen((l) => l.health_score === 3, c.logs),
        7,
      ),
      target: 7,
    }),
  },
  {
    ...
    category: "Surprise Milestones",
    icon: "Pill",
    color: "oklch(0.65 0.16 5)",
    evaluate: (c) => streakLen((l) => allScheduledMedsTaken(l, c.logs), c.logs) >= 30,
    progress: (c) => ({
      current: Math.min(
        streakLen((l) => allScheduledMedsTaken(l, c.logs), c.logs),
        30,
      ),
      target: 30,
    }),
  },
  {
    ...
    progress: (c) => ({
      current: Math.min(
        streakLen((l) => isFormedOnly(l.stool_consistency), c.logs),
        14,
      ),
      target: 14,
    }),
  ...
  },
  {
    ...
    progress: (c) => ({
      current: Math.min(
        countDays((l) => isNoIssues(l.symptoms) && (l.treats?.length ?? 0) === 0, c.logs),
        10,
      ),
      target: 10,
    }),
  ...
  {
    id: "perfectionist_30",
    name: "Data Completeness Vanguard",
...
function sumCompletedWalks(logs: DailyLog[]): number {
  let n = 0;
  for (const l of logs) n += completedWalks(l);
  return n;
}
...
export function evaluateAchievements(ctx: EvalCtx, existing: Set<string>): AchievementDef[] {
  ...
  for (const a of ACHIEVEMENTS) {
    ...
    try {
      if (a.evaluate(ctx)) newly.push(a);
    } catch {
      /* ignore individual eval errors */
    }
  }
  return newly;
}
